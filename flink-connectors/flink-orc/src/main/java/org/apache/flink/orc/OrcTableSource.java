...
import org.apache.flink.orc.OrcReader.Predicate;
...
public class OrcTableSource
	...
	public DataSet<Row> getDataSet(ExecutionEnvironment execEnv) {
		...
		if (predicates != null) {
			for (OrcReader.Predicate pred : predicates) {
	...
	private Predicate toOrcPredicate(Expression pred) {
		if (pred instanceof Or) {
			...
			if (pred instanceof EqualTo) {
				return new OrcReader.Equals(colName, litType, literal);
			} else if (pred instanceof NotEqualTo) {
				return new OrcReader.Not(
					new OrcReader.Equals(colName, litType, literal));
			} else if (pred instanceof GreaterThan) {
				if (literalOnRight) {
					return new OrcReader.Not(
						new OrcReader.LessThanEquals(colName, litType, literal));
				} else {
					return new OrcReader.LessThan(colName, litType, literal);
				}
			} else if (pred instanceof GreaterThanOrEqual) {
				if (literalOnRight) {
					return new OrcReader.Not(
						new OrcReader.LessThan(colName, litType, literal));
				} else {
					return new OrcReader.LessThanEquals(colName, litType, literal);
				}
			...
			if (pred instanceof IsNull) {
				return new OrcReader.IsNull(colName, colType);
			} else if (pred instanceof IsNotNull) {
				return new OrcReader.Not(
					new OrcReader.IsNull(colName, colType));
			} else {
				// unsupported predicate
				LOG.debug("Unsupported predicate [{}] cannot be pushed into OrcTableSource.", pred);
				return null;
