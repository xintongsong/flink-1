...
public class HiveTableInputFormat extends HadoopInputFormatCommonBase<BaseRow, HiveTableInputSplit> {
	...
	private transient HiveReader reader;
	...
	public void open(HiveTableInputSplit split) throws IOException {
		boolean useFlink = parameters.getBoolean(HiveOptions.HIVE_EXEC_ORC_FLINK_READER);
		if (useFlink && useOrcVectorizedRead(split.getHiveTablePartition())) {
			this.reader = new FlinkOrcReader(
					jobConf, partitionKeys, fieldNames, fieldTypes, selectedFields, split);
		} else {
			this.reader = new HiveRecordReader(jobConf, partitionKeys, fieldTypes, selectedFields, split);
		}
	}
	...
	private boolean useOrcVectorizedRead(HiveTablePartition partition) {
		for (int i : selectedFields) {
			switch (fieldTypes[i].getLogicalType().getTypeRoot()) {
				...
				case SYMBOL:
				default:
					LOG.info("Fallback to hadoop record reader, unsupported field type: " + fieldTypes[i]);
					return false;
			}
		}
