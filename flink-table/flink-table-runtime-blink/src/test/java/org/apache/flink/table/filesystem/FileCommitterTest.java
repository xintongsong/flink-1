...
public class FileCommitterTest {
	...
	public void testDynamicPartition() throws Exception {
		...
		committer = new FileCommitter(
				fileSystemFactory, metaStoreFactory,
				false, tmpPath, new HashMap<>(), 2);
	...
	public void testStaticPartition() throws Exception {
		...
		committer = new FileCommitter(
				fileSystemFactory, metaStoreFactory,
				false, tmpPath, partitions, 2);
		createFile("cp-1/task-2/", "f5");
		committer.commitJob(1);
		Assert.assertTrue(new File(outputFile, "p1=0/p2=0/f4").exists());
		Assert.assertTrue(new File(outputFile, "p1=0/p2=0/f5").exists());
	}

	static class TestMetaStoreFactory implements MetaStoreFactory {
		...
		@Override
		public MetaStoreFactory.MetaStore createMetaStore() throws Exception {
			return new MetaStoreFactory.MetaStore() {
				...
				@Override
				public void addPartition(LinkedHashMap<String, String> partSpec,
