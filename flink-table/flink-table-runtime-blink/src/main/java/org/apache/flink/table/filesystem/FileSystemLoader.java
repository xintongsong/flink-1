...
import static org.apache.flink.table.filesystem.FileSystemUtils.listStatusWithoutHidden;
...
public class FileSystemLoader implements Closeable {
	...
	public FileSystemLoader(
			boolean overwrite,
			FileSystem fs,
			MetaStoreFactory factory) throws Exception {
	...
	public void loadPartition(
		...
		Path path = pathFromMeta.orElseGet(() -> new Path(
				metaStore.getLocationPath(), generatePartName(partSpec)));
		overwrite(path);
		renameFiles(srcDirs, path);
	...
	public void loadNonPartition(List<Path> srcDirs) throws Exception {
		...
		overwrite(tableLocation);
		renameFiles(srcDirs, tableLocation);
	}
