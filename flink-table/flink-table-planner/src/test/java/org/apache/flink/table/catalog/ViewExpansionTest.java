...
public class ViewExpansionTest {
	...
	public void testSqlViewExpansion() throws Exception {
		...
		util.verifyJavaSql(
			...
			"StreamTableSourceScan(table=[[builtin, default, tab1]], fields=[], source=[()])");
	}
	...
	public void testSqlViewWithoutFullyQualified() throws Exception {
		...
		util.verifyJavaSql(
			...
			"StreamTableSourceScan(table=[[builtin, different, tab1]], fields=[], source=[()])");
	}
	...
	public void testTableViewWithOnlyDatabaseQualified() throws Exception {
		...
		util.verifyJavaTable(
			tab,
			"StreamTableSourceScan(table=[[different_cat, different_db, tab1]], fields=[], source=[()])");
	}

	@Test
	public void testSqlViewIncompatibleDialect() throws Exception {
		expectedException.expect(ValidationException.class);
		expectedException.expectMessage(
			"Can not query a view with HIVE conformance, with sql parser set to DEFAULT conformance");
		...
		CatalogManager catalogManager = root()
			.builtin(
				database(
					...
					table("tab1"),
					view("view")
						.withQuery("SELECT * FROM `builtin`.`default`.tab1")
						.withDialect(SqlDialect.HIVE)
				)
			).build();
