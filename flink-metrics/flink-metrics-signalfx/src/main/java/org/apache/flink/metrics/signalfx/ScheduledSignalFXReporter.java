...
public class ScheduledSignalFXReporter implements MetricReporter, Scheduled, Reporter, CharacterFilter {
	...
	public void notifyOfAddedMetric(Metric metric, String metricName, MetricGroup group) {
		...
		synchronized (this) {
			if (metric instanceof Counter) {
				...
				if (histogram instanceof SignalFXHistogramWrapper) {
					registry.register(fullName, ((SignalFXHistogramWrapper) histogram).getSignalFXHistogram());
				} else {
				...
				if (meter instanceof SignalFXMeterWrapper) {
					registry.register(fullName, ((SignalFXMeterWrapper) meter).getSignalFXMeter());
				} else {
	...
	@Override
	public void report() {
		// we do not need to lock here, because the SignalFX registry is
		// internally a concurrent map (same as Dropwizard due to com.codahale.metrics.MetricRegistry)
	...
	public ScheduledReporter getReporter(String token) {
		final SignalFxReporter signalfxReporter = new SignalFxReporter.Builder(this.registry, token).build();
