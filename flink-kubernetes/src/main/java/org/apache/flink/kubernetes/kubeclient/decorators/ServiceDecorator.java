 ...
 * setup services port.
 */
public class ServiceDecorator extends Decorator<Service, FlinkService> {

	private KubernetesConfigOptions.ServiceExposedType exposedType;

	private boolean onlyRestPort = false;
	...
	@Override
	protected Service doDecorate(Service resource, Configuration flinkConfig) {
		...
		servicePorts.add(getServicePort(getPortName(RestOptions.PORT.key()),
			flinkConfig.getInteger(RestOptions.PORT)));
		if (!onlyRestPort) {
			servicePorts.add(getServicePort(getPortName(JobManagerOptions.PORT.key()),
				flinkConfig.getInteger(JobManagerOptions.PORT)));
			servicePorts.add(getServicePort(getPortName(BlobServerOptions.PORT.key()), Constants.BLOB_SERVER_PORT));
		}
