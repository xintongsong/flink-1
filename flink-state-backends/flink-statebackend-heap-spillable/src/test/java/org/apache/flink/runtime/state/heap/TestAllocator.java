...
public class TestAllocator implements Allocator {
	...
	public synchronized long allocate(int size) {
		Preconditions.checkArgument(size <= maxAllocateSize,
			"Can't allocate size of " + size + " larger than maxAllocateSize " + maxAllocateSize);
	...
	/**
	 * Implementation of {@link Chunk} used for test. A chunk can only be used
	 * by one space.
	 */
