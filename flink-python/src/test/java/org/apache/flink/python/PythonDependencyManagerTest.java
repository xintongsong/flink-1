...
public class PythonDependencyManagerTest {
	...
	public void prepareDistributeCache() {
		...
		distributeCachedFiles.put("python_requirements_list_file_{uuid}",
			wrapFuture(new Path("/distributed_cache/file2")));
		distributeCachedFiles.put("python_requirements_dir_{uuid}",
	...
	public void testParsePythonFiles() throws IOException {
		...
		PythonDependencyManager dependencyManager =
			PythonDependencyManager.createDependencyManager(pythonFileMetaData, distributedCache);
	...
	public void testParsePythonRequirements() throws IOException {
		Map<String, String> pythonFileMetaData = new HashMap<>();
		pythonFileMetaData.put(PythonDependencyManager.PYTHON_REQUIREMENTS, "python_requirements_list_file_{uuid}");
		PythonDependencyManager dependencyManager =
			PythonDependencyManager.createDependencyManager(pythonFileMetaData, distributedCache);

		assertEquals("/distributed_cache/file2", dependencyManager.getRequirementsFilePath());
		assertNull(dependencyManager.getRequirementsDirPath());

		pythonFileMetaData.put(PythonDependencyManager.PYTHON_REQUIREMENTS_DIR, "python_requirements_dir_{uuid}");
		dependencyManager = PythonDependencyManager.createDependencyManager(pythonFileMetaData, distributedCache);

		assertEquals("/distributed_cache/file2", dependencyManager.getRequirementsFilePath());
		assertEquals("/distributed_cache/file3", dependencyManager.getRequirementsDirPath());
	}
	...
	public void testParsePythonExec() throws IOException {
		...
		assertEquals("/usr/bin/python3", dependencyManager.getPythonExec());
	}
