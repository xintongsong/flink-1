...
public class TaskExecutorPartitionLifecycleTest extends TestLogger {
	...
	public void testJobMasterConnectionTerminationAfterExternalRelease() throws Exception {
		testJobMasterConnectionTerminationAfterExternalReleaseOrPromotion(
			((taskExecutorGateway, jobID, resultPartitionID) -> taskExecutorGateway.releaseOrPromotePartitions(jobID, Collections.singleton(resultPartitionID), Collections.emptyList()))
		);
	}
	...
	public void testJobMasterConnectionTerminationAfterExternalPromotion() throws Exception {
		testJobMasterConnectionTerminationAfterExternalReleaseOrPromotion(
			((taskExecutorGateway, jobID, resultPartitionID) -> taskExecutorGateway.releaseOrPromotePartitions(jobID, Collections.emptyList(), Collections.singleton(resultPartitionID)))
		);
	}
	...
	private void testJobMasterConnectionTerminationAfterExternalReleaseOrPromotion(TriConsumer<TaskExecutorGateway, JobID, ResultPartitionID> releaseOrPromoteCall) throws Exception {
		...
		try {
			...
			taskExecutorGateway.releaseOrPromotePartitions(jobId, Collections.singletonList(new ResultPartitionID()), Collections.emptyList());
	...
	public void testBlockingPartitionReleaseAfterReleaseCall() throws Exception {
		testPartitionRelease(
			(jobId, partitionId, taskExecutorGateway) -> taskExecutorGateway.releaseOrPromotePartitions(jobId, Collections.singletonList(partitionId), Collections.emptyList()),
			true,
			true,
			ResultPartitionType.BLOCKING);
	}
	...
	public void testPromotedBlockingPartitionNotReleasedAfterPromotionCall() throws Exception {
		testPartitionRelease(
			(jobId, partitionId, taskExecutorGateway) -> taskExecutorGateway.releaseOrPromotePartitions(jobId, Collections.emptyList(), Collections.singletonList(partitionId)),
			true,
			false,
			ResultPartitionType.BLOCKING);
	}
	...
	public void testPromotedBlockingPartitionNotReleasedAfterJobMasterDisconnect() throws Exception {
		testPartitionRelease(
			(jobId, partitionId, taskExecutorGateway) -> {
				taskExecutorGateway.releaseOrPromotePartitions(jobId, Collections.emptyList(), Collections.singletonList(partitionId));
