...
public class TestingResourceManagerGateway implements ResourceManagerGateway {
	...
	public void setRegisterTaskExecutorConsumer(Consumer<Tuple5<String, ResourceID, Integer, HardwareDescription, ResourceProfile>> registerTaskExecutorConsumer) {
		this.registerTaskExecutorFunction = tuple5 -> {
			registerTaskExecutorConsumer.accept(tuple5);
	...
	@Override
	public CompletableFuture<RegistrationResponse> registerTaskExecutor(String taskExecutorAddress, ResourceID resourceId, int dataPort, HardwareDescription hardwareDescription, ResourceProfile defaultSlotResourceProfile, Time timeout) {
		...
		if (currentFunction != null) {
			return currentFunction.apply(Tuple5.of(taskExecutorAddress, resourceId, dataPort, hardwareDescription, defaultSlotResourceProfile));
		} else {
			return CompletableFuture.completedFuture(
				new TaskExecutorRegistrationSuccess(
					new InstanceID(),
